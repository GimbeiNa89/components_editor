...
export class Component {
  constructor(config, editor) {
    this.config = config;
    this.editor = editor;
    this.schema = getJsonFileFromPath(
      this.editor.components_path + "/" + config.type + ".json"
    );
  ...
  _dispatchUpdate() {
    document.dispatchEvent(
      new CustomEvent("emit_component_update", {
  ...
  setSelected(selected, e) {
    ...
    if (this.element) {
      this.element.style.zIndex = selected ? 10 : "unset";
    }
  ...
  _dispatchDeleteEvent() {
    document.dispatchEvent(
      new CustomEvent("emit_component_delete", {
