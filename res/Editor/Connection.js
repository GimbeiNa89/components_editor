...
const MENU_ACTIONS_CONFIG = [
  {
    id: "comment",
    icon: "./res/Editor/icons/comment.svg",
    action: (menu) => (menu.data.config.label = CONSTANTS.DEFAULT_LABEL),
  },
  {
    id: "addDot",
    icon: "./res/Editor/icons/add.svg",
    action: (menu) => menu.data.addDot(),
  },
  {
    id: "removeDot",
    icon: "./res/Editor/icons/remove.svg",
    action: (menu) => menu.data.removeDot(),
  },
  {
    id: "removeConnection",
...
class ConnectionMenu extends Menu {
  ...
  _updateColorListState() {
    ...
    this.element
      .querySelector(`[data-color="${activeColor}"]`)
      ?.classList.add("active");
  }

  _renderColorList() {
    const listContainer = createHTMLElement("div", ["list", "color-list"]);
    this.data.editor.colorList.forEach((color) =>
      listContainer.appendChild(this._createColorSpan(color))
    );
  ...
  _renderActions() {
    ...
    MENU_ACTIONS_CONFIG.forEach((action) =>
      listContainer.appendChild(this._createActionItem(action))
    );
    this.element.appendChild(listContainer);
  }
...
export class Connection {
  ...
  _detachGlobalListeners() {
    ...
    document.removeEventListener(
      "component_update",
      this.componentUpdateHandler
    );
    document.removeEventListener(
      "component_delete",
      this.componentDeleteHandler
    );
  }
  ...
  get pinFrom() {
    return this._getReference(this.config.from, "pin");
  }
  get componentFrom() {
    return this._getReference(this.config.from, "component");
  }
  get pinTo() {
    return this._getReference(this.config.to, "pin");
  }
  get componentTo() {
    return this._getReference(this.config.to, "component");
  }

  _onComponentUpdate(event) {
    const componentId = event.detail.componentID;
    if (
      [this.componentFrom?.config.id, this.componentTo?.config.id].includes(
        componentId
      )
    ) {
      this.draw();
  ...
  _ensureSVGContainer() {
    ...
    this.el_svg?.remove();
    this.el_svg = createSVGElement("svg", {
      class: "Connection",
      "data-conn-id": this.config.id,
    });
    this.editor.connectionsContainer.appendChild(this.el_svg);
  }

  _renderPaths(pin1, pin2, shiftX, shiftY) {
    const pathCoords = [pin1, ...(this.config.constrains || []), pin2].filter(
  ...
  _updatePathSegment(line, startPoint, endPoint, shiftX, shiftY) {
    ...
    line.setAttribute(
      "transform",
      `translate(${p1.x}, ${p1.y}) rotate(${angleDeg})`
    );
  ...
  _createDotElement(index) {
    const dot = createSVGElement("circle", {
      class: "Dot",
      r: CONSTANTS.DOT_RADIUS,
      fill: "white",
      "stroke-width": CONSTANTS.STROKE_WIDTH,
      "data-conn-dot-id": index,
    });
    dot.addEventListener("mousedown", (e) =>
      this._handleDotDragStart(e, index)
    );
    return dot;
  }
  ...
  _calculateBoundingBox() {
    const points = [
      this.pinFrom.position,
      this.pinTo.position,
      ...(this.config.constrains || []),
    ...
    if (this.el_svg) {
      Object.assign(this.el_svg.style, {
        left: `${minX}px`,
        top: `${minY}px`,
        width: `${width}px`,
        height: `${height}px`,
    ...
    return {
      pin1: this.pinFrom.position,
      pin2: this.pinTo.position,
      shiftX,
      shiftY,
    };
  }
  ...
  _dispatchUpdateEvent() {
    document.dispatchEvent(
      new CustomEvent("emit_connection_update", {
        detail: {
  ...
  removeDot() {
    const nearDotIndex = this.getNearestDotIndex(
      this.menu.position.x,
  ...
  getNearestDotIndex(posX, posY) {
    ...
    return indexToRemove !== -1 && minDistanceSq <= MIN_DISTANCE_SQ
      ? indexToRemove
      : null;
  }
  ...
  _findNearestSegment(position, allPoints) {
    ...
    for (let i = 0; i < allPoints.length - 1; i++) {
      const segmentDistanceSq = this._distanceToSegmentSq(
        position,
        allPoints[i],
        allPoints[i + 1]
      );
  ...
  _handleUpdate(e) {
    if (e.detail.connectionID === this.config.id) {
  ...
  _dispatchDeleteEvent() {
    document.dispatchEvent(
      new CustomEvent("emit_connection_delete", {
...
export class TempConnection {
  ...
  _createPathElement() {
    ...
    this.el_path = createSVGElement("path", {
      ...
      "stroke-width": CONSTANTS.TEMP_CONNECTION_STROKE,
      fill: "none",
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
    });
  ...
  update(clientX, clientY) {
    ...
    const pin1 = { x: this.startX, y: this.startY },
      pin2 = { x: this.endX, y: this.endY };
    const { shiftX, shiftY, minX, minY, width, height } =
      this._calculateBoundingBox(pin1, pin2);
    ...
    this.el_path.setAttribute(
      "d",
      this._generateCurvePath(pin1, pin2, shiftX, shiftY)
    );
  }
  ...
  _generateCurvePath(pin1, pin2, shiftX, shiftY) {
    const p1x = pin1.x + shiftX,
      p1y = pin1.y + shiftY;
    const p2x = pin2.x + shiftX,
      p2y = pin2.y + shiftY;
