...
export class Pin {
    ...
    get PIN_RADIUS() { return 0; }

    // === GETTERS ===

    get refID() { return `${this.component.config.id}:${this.id}`;}
    get position() {
    ...
    removeEventListeners() {
        ...
        this.component.editor.view.removeEventListener('mousemove', this._handleDragMoveBound);
        this.component.editor.view.removeEventListener('mouseup', this._handleDragEndBound);
    }
    ...
    _createTempConnection() {
        const color = this.component.editor.colorList[this.component.editor.current_color];
    ...
    _attachDragListeners() {
        this.component.editor.container.addEventListener('mousemove', this._handleDragMoveBound);
        this.component.editor.container.addEventListener('mouseup', this._handleDragEndBound);
    }
    ...
    _handleDragEnd(event) {
        ...
        fixMouseCoord(event, this.component.editor.scale, this.component.editor.view);
        this.component.editor.container.style.cursor = "unset";
        
        const targetPin = this._findTargetPin(event.mouseX, event.mouseY);
        
        if (targetPin && targetPin.refID != this.temp_connection.config.from.refID) {
            document.dispatchEvent(new CustomEvent("emit_connection_add", {detail: {config: {
    ...
    _detachDragListeners() {
        this.component.editor.container.removeEventListener('mousemove', this._handleDragMoveBound);
        this.component.editor.container.removeEventListener('mouseup', this._handleDragEndBound);
    }
}