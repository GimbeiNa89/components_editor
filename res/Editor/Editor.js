...
const ACTIONS = [
    {id:"add_component", icon:"./res/Editor/icons/add.svg", label:"Aggiungi componente", action: (d,e) => {
        e.stopPropagation();
        d.showAddComponentPopup()
    }},
    {id:"download", icon:"./res/Editor/icons/download.svg", label:"Scarica diagramma", action: (d,e) => {
       d.downloadDiagram();
    }},
]
...
export class Editor{
    ...
    get colorList(){
        return ['#FF6347', '#3CB371', '#1E90FF', '#FFD700', '#BA55D3', '#FF8C00', '#00CED1', '#FF69B4'];
    }
    ...
    _init(){
        ...
        this.container.addEventListener("click", (e) => { if(e.target.closest(".Editor")) this._initMouseEvents(e, false); });
        this.container.addEventListener("contextmenu", (e) => { if(e.target.closest(".Editor")) this._initMouseEvents(e, true);});
    ...
    _initOverlay(){
        ...
        this.colorList.forEach((c, i) => {
            ...
            e.addEventListener("click", () => {
                ...
                color_overlay.querySelectorAll("span").forEach(c => {c.classList.remove("active")});
    ...
    _initAddComponentPopup(){
        ...
        components_list.forEach(i => {
            ...
            item.addEventListener("click", async (e) => {
                document.dispatchEvent(new CustomEvent("emit_component_add", {detail: {config:{
    ...
    _initMouseEvents(e, openMenu){
        ...
        if(e.target.closest("line") || e.target.closest("circle")){
            const c = this.connections.get(e.target.closest("svg").getAttribute("data-conn-id"));
            e.openMenu = openMenu;
        ...
        if(e.target.closest(".Component")){
            const c = this.components.get(e.target.closest(".Component").getAttribute("data-component-id"));
            e.openMenu = openMenu;
            c.setSelected(true, e);
            return false;
        }
    ...
    // PAN HANDLERS
    _initPanMouseEvents(){
        this.container.addEventListener("mousedown", (e) => { if(e.button == 0) this._handlePanStart(e)});
        this.container.addEventListener("mousemove", (e) => {this._handlePanMove(e)});
        this.container.addEventListener("mouseup", (e) => {this._handlePanEnd(e)});
        this.container.addEventListener("wheel", (e) => {this._handleZoom(e)}, {passive: false});
    }
    _applyTransform(){
        this.view.style.transform = "translate("+this.translateX+"px , "+this.translateY+"px) scale("+this.scale+")"
    }
    _handleZoom(e){
    ...
    async loadDiagram(diagram){
        ...
        this._drawComponents(); this._drawConnections();
    }
    ...
    _getDiagram(){
        ...
        this.components.forEach(c => {components.push(c.config);});
        this.connections.forEach(c => {connections.push(c.config);});
